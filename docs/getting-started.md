...
```toml
[dependencies]
zbox = { version = "0.8.4", features = ["storage-zbox-native"] }
```
...
```toml
[dependencies]
zbox = { version = "0.8.4", features = ["storage-zbox-native", "libsodium-bundled"] }
```
...
6. Open http://localhost:3333/ in browser, remember to open developer tools to
   ...
   ```
   ZboxFS 0.8.4 - Zero-details, privacy-focused in-app file system
   ...[logs omitted]
   ZboxFS exited
   ```
